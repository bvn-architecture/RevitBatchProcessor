        ...
        try:
            ...
            if isCloudModel:
                ...
                if revit_file_util.IsCentralModel(centralFilePath):
                    ...
                    isCentralModel = True
                if centralFileOpenOption == BatchRvtUtil.BatchRvt.CentralFileOpenOption.CreateNewLocal:
            ...
            if activeDoc is not None:
                ...
                if isCloudModel:
                    ...
                    revitVersion = BatchRvtUtil.RevitVersion.GetSupportedRevitVersion(revit_session.GetSessionRevitVersionNumber())
                    localFilePath = BatchRvtUtil.RevitVersion.GetRevitLocalFilePath(revitVersion, centralFilePath)
                    try:
...
def DoRevitSessionProcessing(
    ...
    if len(scriptDatas) > 0:
        ...
        if revitProcessingOption == BatchRvtUtil.BatchRvt.RevitProcessingOption.BatchRevitFileProcessing:
            for scriptData in scriptDatas:
                revit_script_util.SetCurrentScriptData(scriptData)
                if not IsBatchRvtProcessRunning(batchRvtProcessUniqueId):
