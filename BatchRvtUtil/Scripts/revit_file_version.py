...
def InvokeStorageRootMember(storageRoot, methodName, *methodArgs):
    return GetStorageRootType().InvokeMember(
        ...
        | Refl.BindingFlags.Instance
        | Refl.BindingFlags.Public
        | Refl.BindingFlags.NonPublic
...
def GetStorageRoot(filePath):
    storageRoot = (
        ...
        if not str.IsNullOrWhiteSpace(filePath)
        else None
    )
    return storageRoot
...
def GetRevitFileVersionInfoText(revitFilePath):
    ...
    if numberOfTextMarkerIndices == 2:
        ...
        revitVersionInfoText = Encoding.Unicode.GetString(
            bytes[startTextIndex:endTextIndex]
        )
...
def ExtractRevitVersionInfoFromText(revitVersionInfoText):
    ...
    formatLine = indexedLines.SingleOrDefault(
        lambda l: l[1].StartsWith(FORMAT_PROPERTY)
    )
    ...
    if buildLine is not None:
        ...
        revitBuildLine = indexedLines.SingleOrDefault(
            lambda l: l[1].StartsWith(REVIT_BUILD_PROPERTY)
        )
        ...
        if revitBuildLine is None:
            ...
            revitBuildLine = indexedLines.SingleOrDefault(
                lambda l: l[1].Contains(REVIT_BUILD_PROPERTY)
            )
            if revitBuildLine is not None:
                ...
                revitBuildLineText = (
                    revitBuildLineText[:indexOfLastSavePath]
                    if indexOfLastSavePath != -1
                    else revitBuildLineText
                )
        else:
            revitBuildLineText = revitBuildLine[1]
...
def GenerateRevitVersionTextPrefixes(
    revitVersionNumberText, includeDisciplineVersions=False
):
    ...
    return [
        str.Join(" ", prefix, revitVersionNumberText)
        for prefix in REVIT_VERSION_TEXT_PREFIXES
    ]


REVIT_VERSION_TEXT_PREFIXES_2010 = GenerateRevitVersionTextPrefixes(
    "2010", includeDisciplineVersions=True
)
REVIT_VERSION_TEXT_PREFIXES_2011 = GenerateRevitVersionTextPrefixes(
    "2011", includeDisciplineVersions=True
)
REVIT_VERSION_TEXT_PREFIXES_2012 = GenerateRevitVersionTextPrefixes(
    "2012", includeDisciplineVersions=True
)
REVIT_VERSION_TEXT_PREFIXES_2013 = GenerateRevitVersionTextPrefixes(
    "2013", includeDisciplineVersions=True
)
REVIT_VERSION_TEXT_PREFIXES_2014 = GenerateRevitVersionTextPrefixes(
    "2014", includeDisciplineVersions=True
)
REVIT_VERSION_TEXT_PREFIXES_2015 = GenerateRevitVersionTextPrefixes(
    "2015", includeDisciplineVersions=True
)
REVIT_VERSION_TEXT_PREFIXES_2016 = GenerateRevitVersionTextPrefixes(
    "2016", includeDisciplineVersions=True
)
...
def GetRevitVersionNumberTextFromRevitVersionText(revitVersionText):
    ...
    if not str.IsNullOrWhiteSpace(revitVersionText):
        ...
        elif StartsWithOneOfPrefixes(
            revitVersionText, REVIT_VERSION_TEXT_PREFIXES_2011
        ):
            revitVersionNumberText = "2011"
        elif StartsWithOneOfPrefixes(
            revitVersionText, REVIT_VERSION_TEXT_PREFIXES_2012
        ):
            revitVersionNumberText = "2012"
        elif StartsWithOneOfPrefixes(
            revitVersionText, REVIT_VERSION_TEXT_PREFIXES_2013
        ):
            revitVersionNumberText = "2013"
        elif StartsWithOneOfPrefixes(
            revitVersionText, REVIT_VERSION_TEXT_PREFIXES_2014
        ):
            revitVersionNumberText = "2014"
        elif StartsWithOneOfPrefixes(
            revitVersionText, REVIT_VERSION_TEXT_PREFIXES_2015
        ):
            revitVersionNumberText = "2015"
        elif StartsWithOneOfPrefixes(
            revitVersionText, REVIT_VERSION_TEXT_PREFIXES_2016
        ):
            revitVersionNumberText = "2016"
        elif StartsWithOneOfPrefixes(
            revitVersionText, REVIT_VERSION_TEXT_PREFIXES_2017
        ):
            revitVersionNumberText = "2017"
        elif StartsWithOneOfPrefixes(
            revitVersionText, REVIT_VERSION_TEXT_PREFIXES_2018
        ):
            revitVersionNumberText = "2018"
        elif StartsWithOneOfPrefixes(
            revitVersionText, REVIT_VERSION_TEXT_PREFIXES_2019
        ):
            revitVersionNumberText = "2019"
        elif StartsWithOneOfPrefixes(
            revitVersionText, REVIT_VERSION_TEXT_PREFIXES_2020
        ):
            revitVersionNumberText = "2020"
        elif StartsWithOneOfPrefixes(
            revitVersionText, REVIT_VERSION_TEXT_PREFIXES_2021
        ):
            revitVersionNumberText = "2021"
        elif StartsWithOneOfPrefixes(
            revitVersionText, REVIT_VERSION_TEXT_PREFIXES_2022
        ):
            revitVersionNumberText = "2022"
        elif StartsWithOneOfPrefixes(
            revitVersionText, REVIT_VERSION_TEXT_PREFIXES_2023
        ):
            revitVersionNumberText = "2023"
        elif StartsWithOneOfPrefixes(
            revitVersionText, REVIT_VERSION_TEXT_PREFIXES_2024
        ):
            revitVersionNumberText = "2024"
        elif StartsWithOneOfPrefixes(
            revitVersionText, REVIT_VERSION_TEXT_PREFIXES_2025
        ):
